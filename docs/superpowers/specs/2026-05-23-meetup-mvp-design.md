...
create table events (
  ...
  starts_at  timestamptz not null,                  -- 응답 잠금의 기준
  location   text,
  memo       text,
...
| `group_members`             | 같은 그룹의 멤버 (서로 보임)          | 차단 (가입은 `join_group_by_token` RPC만 허용)                                        | × (없음)                                                     | 본인 행만 (탈퇴)                      |
...
| `createEvent`        | `{ groupId, startsAt, location?, memo? }`    | `events` insert (owner 검증은 RLS)                                            |
...
| ID  | 항목                                                          | 대응                                                                                                                            |
| --- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| R1  | `invite_token`이 단톡방에 공개돼 외부인 클릭 가능              | 토큰=가입권은 의도된 동작. RLS는 가입 후에도 그룹 멤버십 행 존재를 이중 검증. 토큰 회전은 v2                                    |
| R2  | `events.starts_at` 타임존 혼동 (서버=UTC vs 사용자=KST)        | 모든 timestamp를 `timestamptz`로 저장. 화면 표시는 `Intl.DateTimeFormat('ko-KR', { timeZone: 'Asia/Seoul' })` 고정              |
